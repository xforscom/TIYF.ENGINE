...
- /health now includes heartbeat_age_seconds, open_positions, active_orders, risk_events_total, alerts_total, `order_rejects_total`, last_decision_utc, timeframes_active, decisions_total, loop_iterations_total, `risk_config_hash`, per-gate block/throttle counters, `idempotency_cache_size`, `idempotency_evictions_total`, `slippage_model`, `promotion_config_hash`, and a nested `promotion` block (candidates, probation_days, min_trades, thresholds) alongside the existing adapter status. Daily monitor runs quote these fields in the summary: daily-monitor: adapter=<name> connected=<bool> heartbeat_age=<xs> stream_connected=<n> stream_heartbeat_age=<xs> bar_lag_ms=<ms> open_positions=<n> active_orders=<n> risk_events_total=<n> alerts_total=<n> last_decision_utc=<iso> timeframes_active=<csv> decisions_total=<n> loop_iterations_total=<n> risk_config_hash=<hash> risk_blocks_total=<n> risk_throttles_total=<n> order_rejects_total=<n> gvrs_raw=<f|n/a> gvrs_ewma=<f|n/a> gvrs_bucket=<bucket|unknown> promotion_candidates=<n|n/a> promotion_probation_days=<n|n/a> promotion_min_trades=<n|n/a>.
- /metrics publishes the same values as gauges/counters (engine_heartbeat_age_seconds, engine_bar_lag_ms, engine_pending_orders, engine_open_positions, engine_active_orders, engine_risk_events_total, engine_alerts_total, engine_order_rejects_total, engine_last_order_size_units{instrument="…"}, engine_stream_connected, engine_stream_heartbeat_age_seconds, engine_loop_uptime_seconds, engine_loop_iterations_total, engine_decisions_total, engine_loop_last_success_ts) and now also emits `engine_idempotency_cache_size` (total plus `{kind}` labels), `engine_idempotency_evictions_total`, `engine_slippage_model{model="…"}`, and the existing `engine_risk_blocks_total{gate="…"}` / `engine_risk_throttles_total{gate="…"}` gauges so Prometheus-compatible scrapers can ingest the rail counters without extra formatting.

### Ops: Kill-switch

- Setting the `TIYF_KILLSWITCH=1` environment variable or creating `/opt/tiyf/kill.switch` blocks new entry orders immediately while still allowing position-reducing exits to flow. Remove the variable (and delete the file) to resume normal operation.
- `ALERT_KILLSWITCH` is rate-limited to one emission per minute while the switch is active; the execution proof and daily monitor artifacts show the throttled cadence.
- `/health` exposes `slippage_model`, `order_rejects_total`, and the idempotency cache gauges so operators can confirm the host state before clearing the switch or re-enabling entries.
...
`daily-monitor: adapter=oanda-demo connected=True heartbeat_age=12.0s stream_connected=0 stream_heartbeat_age=0.5s bar_lag_ms=31056670.7774 open_positions=0 active_orders=0 risk_events_total=52 alerts_total=158 last_decision_utc=2025-10-31T20:00:00Z timeframes_active=H1,H4 decisions_total=100 loop_iterations_total=100 risk_config_hash=abc123def456 risk_blocks_total=4 risk_throttles_total=2 order_rejects_total=0 gvrs_raw=0.1234 gvrs_ewma=0.1178 gvrs_bucket=Calm promotion_candidates=2 promotion_probation_days=30 promotion_min_trades=50`
