...
- GVRS (global volatility regime score) now surfaces alongside the risk counters; it is running in shadow mode for M5, so it emits telemetry and alerts but does not block or resize orders yet.
